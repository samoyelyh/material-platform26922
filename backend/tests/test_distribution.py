...
def _dispatch(client, pkg_id: str, operator_id: str = "zhang", operator_name: str = "张三") -> dict:
    response = client.post(
        f"/api/design-packages/{pkg_id}/distributions",
        json={"operatorId": operator_id, "operatorName": operator_name, "actor": "肖芸"},
    )
    assert response.status_code == 200, response.text
    return response.json()
...
def test_dispatch_receive_cancel_bind_full_flow(client, phase2_package, db_session):
    ...
    # 列表
    lst = client.get(f"/api/design-packages/{ctx['pkg']['id']}/distributions")
    ...
    # 取消（完成后取消仍是有效业务操作）
    cancelled = client.post(f"/api/distributions/{task_id}/cancel")
...
def test_dispatch_duplicate_active_rejected(client, phase2_package):
    ...
    # 取消后可重新派发给同一运营
    task = client.get(f"/api/design-packages/{ctx['pkg']['id']}/distributions").json()[0]
    cancel = client.post(f"/api/distributions/{task['id']}/cancel")
    assert cancel.status_code == 200
    again = _dispatch(client, ctx["pkg"]["id"])
    assert again["id"] != task["id"]


def test_dispatch_requires_batch(client):
    """没有上架版本的设计包不能派发。"""
    response = client.post(
        "/api/design-packages/nonexistent/distributions",
        json={"operatorId": "zhang", "operatorName": "张三"},
    )
    assert response.status_code == 404


def test_bind_asins_validation(client, phase2_package):
...
def test_global_distributions_list_and_status_filter(client, phase2_package):
    ...
    all_tasks = client.get("/api/distributions")
    assert all_tasks.status_code == 200, all_tasks.text
    assert len(all_tasks.json()) == 1
    assert all_tasks.json()[0]["id"] == task["id"]

    active = client.get("/api/distributions?status=ACTIVE")
    assert len(active.json()) == 1
    received = client.get("/api/distributions?status=RECEIVED")
    assert received.json() == []

    # 接收后状态变化反映到筛选
    client.post(f"/api/distributions/{task['id']}/receive")
    assert client.get("/api/distributions?status=RECEIVED").json()[0]["status"] == "RECEIVED"
    assert client.get("/api/distributions?status=ACTIVE").json() == []


def test_download_task_zip_uses_snapshot(client, phase2_package, db_session):
    ...
    resp = client.get(f"/api/distributions/{task['id']}/download")
...
def test_variant_detail_includes_distribution_asins(client, phase2_package):
    ...
    # 取消后不再展示该任务
    client.post(f"/api/distributions/{task['id']}/cancel")
